...
START_TEST(test_dmm_fs9721_parser)
{
	size_t test_idx;
	float floatval;
	...
	for (test_idx = 0; test_idx < ARRAY_SIZE(test_params); test_idx++) {
		...
		memset(&info, 0, sizeof(struct fs9721_info));

		ret = sr_fs9721_parse(
			test_param->input_buf, &floatval, &analog, &info);

		fail_unless(ret == SR_OK,
			"sr_fs9721_parse() for '%s' failed with %d.",
			test_param->desc, ret);
...
SR_PRIV void register_tests_fs9721(Suite *s)
{
	...
	tc = tcase_create("fs9721");
	tcase_add_test(tc, test_dmm_fs9721_parser);
