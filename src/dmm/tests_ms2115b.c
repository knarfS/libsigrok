...
START_TEST(test_dmm_ms2115b_parser)
{
	size_t test_idx;
	float floatval;
	...
	for (test_idx = 0; test_idx < ARRAY_SIZE(test_params); test_idx++) {
		...
		memset(&info, 0, sizeof(struct ms2115b_info));

		ret = sr_ms2115b_parse(
			test_param->input_buf, &floatval, &analog, &info);

		fail_unless(ret == SR_OK,
			"sr_ms2115b_parse() for '%s' failed with %d.",
			test_param->desc, ret);
...
SR_PRIV void register_tests_ms2115b(Suite *s)
{
	TCase *tc;

	tc = tcase_create("ms2115b");
	tcase_add_test(tc, test_dmm_ms2115b_parser);
